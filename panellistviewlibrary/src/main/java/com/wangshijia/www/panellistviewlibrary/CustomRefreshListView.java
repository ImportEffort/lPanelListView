...
public class CustomRefreshListView extends ListView implements AbsListView.OnScrollListener {
    ...
    private void initHeaderView() {
        ...
        iv_arrow = (ImageView) headerView.findViewById(R.id.pull_to_refresh_header_arrow);
        pb_rotate = (ProgressBar) headerView.findViewById(R.id.pull_to_refresh_header_progressbar);
        tv_state = (TextView) headerView.findViewById(R.id.pull_to_refresh_header_hint_textview);
        tv_time = (TextView) headerView.findViewById(R.id.pull_to_refresh_last_update_time_text);
    ...
    private void initFooterView() {
        footerView = View.inflate(getContext(), R.layout.pull_to_load_footer, null);
    ...
    public void completeRefresh() {
        if (isLoadingMore) {
            ...
            tv_state.setText("下拉刷新");
            tv_time.setText(String.format("最后刷新：%s", getCurrentTime()));
        }
    }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == OnScrollListener.SCROLL_STATE_IDLE
                && getLastVisiblePosition() == (getCount() - 1) && !isLoadingMore && isLoadingMoreEnable) {
            isLoadingMore = true;
            //显示出footerView
            footerView.setPadding(0, 0, 0, 0);
            //让listview最后一条显示出来，在页面完全显示出底布局
